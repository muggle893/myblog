...
const aboutParagraphs = computed(() => (appState.profile.aboutBody || '').split(/\n\s*\n/).filter(Boolean))
const dialog = ref(null), avatarFileInput = ref(null), coverFileInput = ref(null)
const saving = ref(false), coverSrc = ref('')
const form = reactive({ nickname: '', blogTitle: '', heroTitle: '', heroSubtitle: '', signature: '', aboutTitle: '', aboutBody: '', aboutTagsText: '', coverPosition: 48 })
...
let coverToken = 0, disposed = false
...
const previewCoverStyle = computed(() => imageStyle(previews.cover || defaultCover, form.coverPosition))

function imageStyle(src, position) {
  return { backgroundImage: src ? `url("${src}")` : undefined, backgroundPosition: `center ${Math.min(100, Math.max(0, Number(position) || 0))}%` }
}
...
async function loadPreview(kind) {
  ...
  try {
    ...
    if (!disposed && dialog.value?.open && token === previewTokens[kind]) previews[kind] = url
  } catch {}
}
function openDialog() {
  if (!appState.owner || saving.value) return
  clearDraftImages()
  Object.assign(form, appState.profile, { aboutTagsText: (appState.profile.aboutTags || []).join('，') })
...
function selectImage(kind, event) {
  ...
  if (file.size > 20 * 1024 * 1024) { toast('图片请控制在 20 MB 以内'); return }
  previewTokens[kind]++
...
async function submitProfile() {
  if (saving.value) return
  if (!appState.owner) { toast('请先登录自己的账号'); return }
  if (!form.nickname.trim() || !form.blogTitle.trim() || !form.heroTitle.trim() || !form.aboutTitle.trim()) {
    toast('昵称、博客名称、首页标题和介绍标题不能为空'); return
  }
  const tags = [...new Set(form.aboutTagsText.split(/[,，\n]/).map(tag => tag.trim()).filter(Boolean))]
  if (tags.length > 12 || tags.some(tag => Array.from(tag).length > 20)) { toast('最多填写 12 个标签，每个标签不超过 20 个字'); return }
  const current = { ...appState.profile }
  const next = { ...current, aboutTags: tags, coverPosition: Number(form.coverPosition) }
  for (const key of ['nickname', 'blogTitle', 'heroTitle', 'heroSubtitle', 'signature', 'aboutTitle', 'aboutBody']) next[key] = form[key].trim()
  const newIds = []
  saving.value = true
  try {
    for (const kind of ['avatar', 'cover']) {
    ...
    if (disposed || !appState.owner) throw new Error('编辑已结束或登录已失效，请重新登录后保存')
    if (!saveProfile(next)) throw new Error('浏览器无法保存个人资料，请检查存储空间后重试')
    // 资料保存成功后才能清理旧图片，取消和保存失败均保留原资料。
    discardImages(['avatarImageId', 'coverImageId'].filter(key => current[key] && current[key] !== next[key]).map(key => current[key]))
    saving.value = false
    closeDialog()
...
function accountAction() {
  if (appState.owner) { logoutOwner(); router.replace('/') }
  else router.push({ name: 'login', query: { next: '/profile' } })
}
onBeforeUnmount(() => { disposed = true; coverToken++; clearDraftImages() })
</script>
<template>
<main class="wrap">
  ...
  <section class="card">
    ...
    <div class="profile-main">
      <div class="profile-intro"><div class="profile-name"><AvatarView size="xl"/><h1>{{ appState.profile.nickname }}</h1><p>@frank · 学习中的开发者</p></div><RouterLink class="btn" :to="appState.owner?'/editor':'/'"><Icon :name="appState.owner?'pen':'book'"/>{{ appState.owner?'写一篇文章':'查看公开文章' }}</RouterLink></div>
      <div class="profile-grid">
        <div class="profile-about"><h2 class="section-title">{{ appState.profile.aboutTitle }}</h2><p v-for="(paragraph, index) in aboutParagraphs" :key="index" class="profile-about-paragraph">{{ paragraph }}</p><div v-if="appState.profile.aboutTags.length" class="tags"><span v-for="tag in appState.profile.aboutTags" :key="tag" class="tag">{{ tag }}</span></div></div>
  ...
  <dialog ref="dialog" class="profile-dialog" aria-labelledby="profile-dialog-title" @cancel="onCancel" @close="clearDraftImages">
    <form @submit.prevent="submitProfile">
      <fieldset class="profile-form-fields" :disabled="saving">
        <div class="dialog-heading"><h2 id="profile-dialog-title">编辑个人资料</h2><button type="button" class="icon-button" aria-label="关闭" @click="closeDialog">×</button></div>
        ...
        <input ref="coverFileInput" type="file" accept="image/png,image/jpeg,image/webp,image/gif" hidden @change="selectImage('cover', $event)">
        <div class="profile-image-actions"><button type="button" class="btn secondary" @click="coverFileInput?.click()">更换封面</button><button type="button" class="btn outline" @click="resetImage('cover')">恢复默认封面</button></div>
        <label class="cover-position-label">画面上下位置<input v-model.number="form.coverPosition" type="range" min="0" max="100" aria-label="封面画面上下位置"></label>
        <small class="profile-edit-hint">建议选择横向图片。支持 PNG、JPG、WebP、GIF，单张不超过 20 MB。</small>
        <h3 class="profile-editor-heading">头像</h3>
        <div class="avatar-editor"><div class="avatar xl avatar-preview" :class="{'has-photo':previews.avatar}"><img v-if="previews.avatar" :src="previews.avatar" alt="头像预览"><template v-else>{{ fallbackLetter() }}</template></div><div class="avatar-editor-actions"><input ref="avatarFileInput" type="file" accept="image/png,image/jpeg,image/webp,image/gif" hidden @change="selectImage('avatar', $event)"><div><button type="button" class="btn secondary" @click="avatarFileInput?.click()">选择本地图片</button><button type="button" class="btn outline" @click="resetImage('avatar')">移除头像</button></div><small>头像、昵称会同步到首页、顶部导航和个人主页。</small></div></div>
        <h3 class="profile-editor-heading">自我介绍</h3>
        <label>介绍标题<input v-model="form.aboutTitle" maxlength="60" required></label>
        <label>介绍正文<textarea v-model="form.aboutBody" maxlength="3000" rows="6" placeholder="介绍一下自己、正在学习的知识，或想记录的生活……"></textarea></label>
        <small class="profile-edit-hint">支持换行与空行分段，最多 3000 字。</small>
        <label>个人标签<input v-model="form.aboutTagsText" maxlength="300" placeholder="Java，SSM，Vue，持续学习"></label>
        ...
        <div class="dialog-actions"><button type="button" class="btn outline" @click="closeDialog">取消</button><button type="submit" class="btn">{{ saving ? '正在保存…' : '保存修改' }}</button></div>
      </fieldset>
    </form>
  </dialog>
</main>
